...
const handleBack = () => {
  if (!idProc) {
    ...
    return;
       ...
       router.push(`/renouvellement/step6/page6?id=${idProc}&originalDemandeId=${originalId}&original_proc_id=${originalProcIdStr}`);
  ...
  const fetchData = async () => {
    ...
    try {
      const [procRes,detenteur, seancesBasicRes, seancesWithDecRes] = await Promise.all([
        ...
        axios.get(`${apiURL}/api/procedures/${originalProcIdStr}/demande`),
